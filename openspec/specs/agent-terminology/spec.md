...
Defines the naming convention for user-facing text describing the chat bot concept: rendered UI text refers to it as "Agent(s)", not "Chat Bot(s)"/"ChatBot(s)"/"Chatbot(s)". This governs page titles, headings, navigation labels, empty states, confirmation dialogs, and descriptive text across the public chat surface and the admin AI tools surface — as distinct from route paths, URLs, class/file/prop names, and JSON/API field names, which this spec does not govern.
