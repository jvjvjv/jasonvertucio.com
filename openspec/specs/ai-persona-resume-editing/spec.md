...
- **AND** a user with `edit-resume` approves one of them
...
- **WHEN** a user with `edit-resume` approves a `pending` candidate
- **THEN** a new resume version is created and becomes the current version
...
- **WHEN** a candidate's data is successfully written to a new live resume version
