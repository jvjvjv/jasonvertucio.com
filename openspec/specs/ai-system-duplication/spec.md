# ai-system-duplication

## Purpose

Defines how a duplicated AiSystem is created, how its "pending first edit" state is represented and surfaced to the Edit page so Provider/Model/API Key can be changed once, and how that state is cleared on first save — the app's only way to change an existing system's provider, model, or API key.
