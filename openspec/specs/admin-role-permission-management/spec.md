...
Defines the admin dashboard UI for managing Keystone roles and permissions — listing, creating, editing, and deleting roles and permissions, and choosing which permissions each role holds. Every action is gated on the corresponding seeded Keystone permission (`view-roles`, `create-roles`, `edit-roles`, `delete-roles`, `view-permissions`, `create-permissions`, `edit-permissions`, `delete-permissions`, `assign-permissions`). Role and permission `name` values are immutable after creation because code references them directly in `can:` checks, and deletion is blocked while a role still has users or a permission is still held by a role.
