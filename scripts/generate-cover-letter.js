 ...
 *
 * Uses docxtemplater with the built-in rawxml module to inject formatted
 ...
 *   {greeting}        - plain text
 *   {@messageBody}    - raw OOXML injected by rawxml module (Markdown → OOXML)
 *   {closing}         - plain text
 *   {signature}       - plain text
 *
 * Usage: node generate-cover-letter.js <templatePath> <dataJsonPath> <outputPath>
 *
...
if (args.length !== 3) {
    console.log(JSON.stringify({
        success: false,
        error: 'Invalid arguments. Usage: node generate-cover-letter.js <templatePath> <dataJsonPath> <outputPath>'
    }));
    process.exit(1);
}
...
try {
    ...
    // Inject list numbering definitions if the template doesn't have them.
    // Our OOXML uses numId=1 for bullets and numId=2 for ordered lists.
...
<w:numbering xmlns:wpc="http://schemas.microsoft.com/office/word/2010/wordprocessingCanvas"
  ...
  <w:num w:numId="2"><w:abstractNumId w:val="1"/></w:num>
        ...
        // Add the relationship in document.xml.rels
        ...
        // Add proper content type for numbering.xml in [Content_Types].xml
