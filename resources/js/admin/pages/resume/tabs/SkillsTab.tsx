...
export default function SkillsTab({ skills, onChange }: SkillsTabProps) {
    const [reorderOpen, setReorderOpen] = useState<'top' | 'other' | null>(null);
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                            ...
                            <Box sx={{ display: 'flex', gap: 1 }}>
                                {skills[type].length >= 2 && (
                                    <Button size="small" onClick={() => setReorderOpen(type)}>
                                        Reorder
                                    </Button>
                                )}
                                <Button
                                    size="small"
                                    onClick={() => onChange(type, [...skills[type], { title: '', list: [] }])}
                                >
                                    + Add Category
                                </Button>
                            </Box>
                            ...
                            <Box key={catIdx} sx={{ mb: 3, p: 2, bgcolor: 'grey.50', borderRadius: 1 }}>
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                                    ...
                                    <IconButton
                                        ...
                                        size="small"
                                        onClick={() => {
                                            const updated = skills[type].filter((_, i) => i !== catIdx);
                                            onChange(type, updated);
                                        }}
                                    >
                                        ✕
            ...
            {reorderOpen && (
                <ReorderModal
                    ...
                    onReorder={(ids) => handleReorder(reorderOpen, ids)}
                />
            )}
        </Box>
    );
