...
export default function SiteSettingsEditor({ links: initialLinks, permissions }: EditorProps) {
    const [links, setLinks] = useState<InternalLink[]>(() => toInternal(initialLinks));
    ...
    const toggleCard = useCallback((index: number) => {
        setLinks((prev) => prev.map((l, i) => (i === index ? { ...l, _open: !l._open } : l)));
    }, []);

    const updateLink = useCallback((index: number, field: string, value: string) => {
    ...
    const addLink = useCallback(() => {
        setLinks((prev) => [
            ...prev,
            { _id: idCounter++, _open: true, label: '', href: '', ariaLabel: '', hover: '', target: '', can: '' },
        ]);
    }, []);

    const addDivider = useCallback(() => {
        setLinks((prev) => [...prev, { _id: idCounter++, _open: false, divider: true }]);
    }, []);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (json.status === 'success') {
                setSnackbar({ open: true, severity: 'success', message: 'Navigation saved.' });
            } else {
                setSnackbar({ open: true, severity: 'error', message: json.message ?? 'Save failed.' });
            }
        } catch {
        ...
        <AdminLayout>
            ...
            <Stack spacing={1.5} sx={{ mb: 3 }}>
                {links.map((link, index) => (
                    <Card key={link._id} sx={link._open ? { borderColor: 'primary.main' } : {}}>
                        {/* Header row */}
                        <Box
                        ...
                        >
                            <DragIndicatorIcon fontSize="small" sx={{ color: 'grey.400' }} />
                            ...
                            >
                                {link.divider ? '— divider —' : link.label || '(new link)'}
                            </Typography>
                            ...
                            {link._open ? (
                                <ExpandLessIcon fontSize="small" sx={{ color: 'grey.400' }} />
                            ) : (
                                <ExpandMoreIcon fontSize="small" sx={{ color: 'grey.400' }} />
                            )}
                            ...
                            <Box sx={{ px: 2, py: 2 }}>
                                {link.divider ? (
                                    <Typography variant="body2" color="text.secondary" fontStyle="italic">
                                        This is a visual divider. It renders as a horizontal rule in the navigation dropdown.
                                    </Typography>
                                ) : (
                                    <Box sx={{ display: 'grid', gap: 2, gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' } }}>
                                        ...
                                        <TextField
                                            ...
                                            onChange={(e) => updateLink(index, 'ariaLabel', e.target.value)}
                                        ...
                                        <TextField
                                            ...
                                            size="small"
                                            value={link.hover ?? ''}
                                            onChange={(e) => updateLink(index, 'hover', e.target.value)}
                                        ...
                                        <TextField
                                            ...
                                            size="small"
                                            value={link.can ?? ''}
                                            onChange={(e) => updateLink(index, 'can', e.target.value)}
            ...
            <Box sx={{ display: 'flex', gap: 2, mb: 4 }}>
                ...
                <Button variant="outlined" color="inherit" startIcon={<RemoveIcon />} onClick={addDivider}>
                    Add divider
                </Button>
            </Box>
