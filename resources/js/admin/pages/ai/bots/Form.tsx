...
export default function Form({ data, setData, errors, systems, roles }: AiChatBotFormProps) {
    return (
        <>
            ...
            <TextField
                ...
                onChange={(event) => setData('ai_system_id', parseInt(event.target.value, 10) || '')}
            ...
            <TextField
                ...
                onChange={(event) => setData('prompt_template', event.target.value)}
                error={!!errors.prompt_template}
                helperText={errors.prompt_template || 'Supported placeholders: {{bot_name}}, {{bot_slug}}, {{bot_description}}, {{visitor_name}}, {{visitor_email}}'}
                sx={{ mb: 2 }}
            />
            ...
            <Box sx={{ mb: 2 }}>
                ...
                <TextField
                    ...
                    onChange={(event) => {
                        ...
                        setData('allowed_roles', [...data.allowed_roles, value]);
                    }}
                    helperText={errors.allowed_roles || 'Leave empty to allow any authenticated role when the bot is not public.'}
                >
                ...
                <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mt: 1 }}>
                    {data.allowed_roles.map((role) => (
                        <Chip
                            key={role}
                            label={role}
                            onDelete={() => setData('allowed_roles', data.allowed_roles.filter((item) => item !== role))}
                        />
                    ))}
                </Box>
            </Box>
            ...
            <Box sx={{ display: 'grid', gap: 1 }}>
                <FormControlLabel
                    control={<Checkbox checked={data.is_active} onChange={(event) => setData('is_active', event.target.checked)} />}
                    label="Active"
                />
                <FormControlLabel
                    control={<Checkbox checked={data.is_public} onChange={(event) => setData('is_public', event.target.checked)} />}
                    label="Public"
                />
                <FormControlLabel
                    control={<Checkbox checked={data.require_visitor_identity} onChange={(event) => setData('require_visitor_identity', event.target.checked)} />}
